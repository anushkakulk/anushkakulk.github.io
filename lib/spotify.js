...
export const getArtistIds = async () => {
  ...
  const artistIds = items.map((track) => track.artists[0].id).join(",");

  return artistIds;
};

export const recommededTracks = async () => {
  const { access_token } = await getAccessToken();
  const artistIds = await getArtistIds();
  const timestamp = Date.now();
  return fetch(`https://api.spotify.com/v1/recommendations?timestamp=${timestamp}limit=5&seed_artists=${artistIds}`, {
    headers: {
      Authorization: `Bearer ${access_token}`,
    },
  });
